...
describe("Subscriptions", function() {
  ...
  describe("RNLocation.subscribeToLocationUpdates", function() {
    let listener;
    let removeSpy;
    ...
    it("should return a function to unsubscribe with", async function() {
      const unsubscribe = await jet.module.subscribeToLocationUpdates(listener);
      expect(unsubscribe).to.be.a("function");
    });

    it("should call addListener when subscribed to", async function() {
      await jet.module.subscribeToLocationUpdates(listener);
    ...
    it("should call to start location updates when subscribed to", async function() {
      await jet.module.subscribeToLocationUpdates(listener);
      expect(nativeInterface.startUpdatingLocation).to.have.been.calledOnce;
      expect(nativeInterface.stopUpdatingLocation).not.to.have.been.called;
    });
    ...
    it("should call to stop location updates when the unsubscribe function is called with a single subscriber", async function() {
      ...
      expect(nativeInterface.stopUpdatingLocation).to.have.been.calledOnce;
    });
    ...
    it("should not call to stop location updates if there is still another subscriber when an unsubscribe is called", async function() {
      const unsubscribe1 = await jet.module.subscribeToLocationUpdates(
        listener
      );
      const unsubscribe2 = await jet.module.subscribeToLocationUpdates(
        listener
      );
      unsubscribe1();
